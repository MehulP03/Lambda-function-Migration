...
def create_function(function_name, function_handler, function_runtime):
    ...
    SECRET_KEY = acct_b['Credentials']['SecretAccessKeyId']
...
def get_function_from_source():
    ...
    for response in response_iterator:
        ...
        for function in functions:
            ...
            create_function(function_name, function_handler, function_runtime)
...
def export_lambda_code(lambda_client, function_name):
    response = lambda_client.get_function(FunctionName=function_name)
    code_data = response['Code']
    with open('./' + function_name + '.zip', 'wb') as zip_file:
        zip_file.write(code_data['Location'].read())
